...
const EditRedemptionModal = (props) => {
  ...
  const submit = async (values) => {
    ...
    if (!localInputs.random_quota) {
      ...
      localInputs.quota = localInputs.min_quota;
    }
                    ...
                    <Row gutter={12}>
                      <Col span={12}>
                        <Form.InputNumber
                          ...
                          min={1}
                          rules={[
                            { required: true, message: t('最小额度必须大于0') },
                            {
                              validator: (rule, v) => {
                                const num = parseInt(v, 10);
                                return num > 0
                                  ? Promise.resolve()
                                  : Promise.reject(t('最小额度必须大于0'));
                              },
                            },
                          ]}
                          style={{ width: '100%' }}
                          extraText={renderQuotaWithPrompt(
                            Number(values.min_quota) || 0,
                          )}
                          showClear
                        />
                      </Col>
                      <Col span={12}>
                        <Form.InputNumber
                          ...
                          min={1}
                          rules={[
                            { required: true, message: t('最大额度必须大于0') },
                            {
                              validator: (rule, v) => {
                                const num = parseInt(v, 10);
                                return num > 0
                                  ? Promise.resolve()
                                  : Promise.reject(t('最大额度必须大于0'));
                              },
                            },
                            {
                              validator: (rule, v) => {
                                const maxValue = parseInt(v, 10);
                                const minValue = parseInt(values.min_quota, 10);
                                if (!minValue || !maxValue) {
                                  return Promise.resolve();
                                }
                                return minValue <= maxValue
                                  ? Promise.resolve()
                                  : Promise.reject(t('最小额度不能大于最大额度'));
                              },
                            },
                          ]}
                          style={{ width: '100%' }}
                          extraText={renderQuotaWithPrompt(
                            Number(values.max_quota) || 0,
                          )}
                          showClear
                        />
                      </Col>
